...
impl MappedFile {
        ...
        match (
            ...
            (Some(Some(base_name)), Some(parent)) => {
                let mut files = fs::read_dir(parent)?
                    .filter_map(|o| o.ok())
                    .filter_map(|o| {
                        if self.path.extension() == o.path().extension() && matches!(o.file_name().to_str(), Some(s) if s.starts_with(base_name.as_str())) {
                            Some(o.path())
                        } else {
                            None
                        }
    ...
    fn backup_files(&mut self) -> io::Result<()> {
        if let (Some(base_name), Some(parent)) = (self.path.file_stem(), self.path.parent()) {
            ...
            // would rather rename than copy
            _ = self.file.take();
            fs::rename(self.path.as_path(), new_path)?;
        }
...
mod tests {
    ...
    pub fn test_write_file() -> io::Result<()> {
        let file_path = format!("./abc-{:#x}.txt", Local::now().timestamp());
        ...
        file.write_all(b"dd")?;
        assert_eq!(file.mapped_files().unwrap().len(), 4);
        file.write_all(b"ee")?;
        assert_eq!(file.mapped_files().unwrap().len(), 4);
